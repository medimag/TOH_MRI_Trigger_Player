...
DEFAULT_AUDIO_FILE = "/Users/gmelkus/Projetcs/MRI_Music/Bore_2_AXL_DWI_ambient.mp3"
DEFAULT_PORT = "/dev/cu.usbserial-FTJRNVL8"
LOGO_FILE = "/Users/gmelkus/Projetcs/MRI_Music/The_Ottawa_Hospital_Logo.jpg"


class TriggerMusicGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("TOH MRI Music Synchronization System")
    ...
    def add_logo(self, parent):
        try:
            if not os.path.exists(LOGO_FILE):
                raise FileNotFoundError(LOGO_FILE)
    ...
    def choose_file(self):
        file_path = filedialog.askopenfilename(
            ...
            filetypes=[
                ("Audio files", "*.mp3 *.wav *.m4a *.aiff *.aac"),
                ("All files", "*.*")
            ]
        )
        if file_path:
    ...
    def play_audio(self):
        if pygame is None:
            self.queue_log("ERROR: pygame is not installed. Run: pip install pygame")
            return
    ...
    def draw_timeline(self):
        ...
        for i, trigger_time in enumerate(recent_times):
            ...
            if i == 0 and len(self.trigger_times) == len(recent_times):
                label = "T1"
            else:
                original_index = len(self.trigger_times) - len(recent_times) + i + 1
                label = f"T{original_index}"

            self.timeline_canvas.create_text(x, 10, text=label, font=("Arial", 8))

            if i == 0:
                self.timeline_canvas.create_text(x, 60, text="0.000s", font=("Arial", 8))
            else:
                dt = trigger_time - recent_times[i - 1]
                self.timeline_canvas.create_text(x, 60, text=f"{dt:.3f}s", font=("Arial", 8))
    ...
    def start_monitoring(self):
        ...
        if self.simulate_mode.get():
            self.port_status_label.config(text="Port Status: Simulated mode")
            self.monitor_thread = threading.Thread(target=self.simulated_trigger_monitor, daemon=True)
        else:
            self.port_status_label.config(text="Port Status: Connecting...")
    ...
    def stop_monitoring(self):
        ...
        if not self.simulate_mode.get():
            self.port_status_label.config(text="Port Status: Not connected")
    ...
    def handle_trigger(self, pin_state):
        ...
        self.queue_log(
            f"Trigger #{self.trigger_count} detected "
            f"(Pin state: {pin_state})"
        )
    ...
    def real_trigger_monitor(self):
        if serial is None:
            self.queue_log("ERROR: pyserial is not installed. Run: pip install pyserial")
            return
        ...
        self.gui_queue.put(("log", "Waiting for real fMRI triggers..."))
